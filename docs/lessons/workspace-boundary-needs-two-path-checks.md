...
应把 ownership 交给后台 supervisor，做有界 termination retry，之后继续持有并最终 reap；
当前调用则明确报告仍可能存在 live command。fail-closed 的非 Unix 路径也必须能通过编译，
...
返回成功，也要先确认 leader terminal，并在保留 PGID reservation 时再次 seal residual group，
最后才能 reap。否则 macOS 的 partial signal success 或 leader 先退出竞态仍会泄漏同组成员。
