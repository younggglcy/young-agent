...
Read/Search 的 path 与 query 必须在转成 `Path` 或进入 workspace resolution 前做输入限长；
Search 的扫描预算也不等于输出构建预算：path/query metadata 还要做序列化限长，
...
可见内容 buffer，避免短行密集文件为每一行重新分配。不支持安全原子 patch 的平台应在
patch 输入基本限长后立即 preflight，不能等到解析、读取、hash 和构造输出完成后才拒绝。
...
`CommandProcess` 也不能用 `DerefMut` 暴露 raw child，或把 seal 与 reap 作为两个独立方法；
只有单一的 `seal_and_reap_terminal_group` 能在 token EOF 后执行 wait。cleanup 使用 lifecycle
enum 区分 already-reaped 与 terminate-and-reap，不能用两个布尔值表示非法组合。token endpoint
可能被后代写入，因此每次观察还
...
cleanup 的首次 group signal 若返回 `EPERM`，但随后确认 leader terminal、tracking token EOF 且
...
`poll` 遇到 `EINTR` 时必须立即把控制权还给拥有绝对 deadline 的外层循环，不能用原始 timeout
无限重试；否则持续信号可以把名义上有界的 drain 变成无界等待。
...
residual child 应保持长时间存活；race 测试在同步成功时依赖 token EOF，在 supervisor handoff 时
等待对应 completion barrier。wire-level 测试只验证成功或明确的 supervised fail-closed，不冒充
cleanup completion 证明。需要证明 escaped child 存活时，则用 ready/release 文件握手，并以原子
rename 发布结果，不用“先睡一段再看 marker”。
command cancellation 也应先等 shell 写出 ready marker 再触发；search cancellation 则用可控
reader 在首次读取后翻转 cancellation，证明扫描已经开始，而不是用大文件和固定延迟猜测 in-flight 状态。
...
Issue #8 已把 read-only 与 mutating command 的细粒度分类放进独立
`command_policy`；它决定是否需要 approval，但不把 cwd binding 描述成 sandbox。
