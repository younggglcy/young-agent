...
needs two post-fork hooks that Rust exposes through unsafe `CommandExt::pre_exec`: binding the
child cwd with `fchdir` on an already-open directory capability, and enabling Linux
...
owns a cloned directory handle for the full lifetime of the registered hook, and both hooks are
limited to async-signal-safe syscalls with no allocation or shared-state access after fork.
